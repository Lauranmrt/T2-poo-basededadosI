...
-- TRIGGER 1: INSERT
CREATE TRIGGER trg_auditoria_emprestimo_insert AFTER INSERT ON emprestimo FOR EACH ROW
BEGIN
    DECLARE v_nome_leitor VARCHAR(250);
    DECLARE v_titulo_obra VARCHAR(500);
    SELECT nome_usuario INTO v_nome_leitor FROM usuario WHERE id = NEW.usuario_id;
    SELECT o.titulo INTO v_titulo_obra FROM exemplar e JOIN obra o ON e.obra_id = o.id WHERE e.id = NEW.exemplar_id;
    -- Grava CURRENT_USER() padrão do banco
    INSERT INTO log_auditoria (acao, usuario_id, usuario_nome, exemplar_id, titulo_exemplar, detalhes, data_hora)
    VALUES ('NOVO EMPRÉSTIMO', NEW.usuario_id, CURRENT_USER(), NEW.exemplar_id, v_titulo_obra, CONCAT('Leitor: ', v_nome_leitor), NOW());
END $$

CREATE TRIGGER trg_auditoria_emprestimo_update AFTER UPDATE ON emprestimo FOR EACH ROW
BEGIN
    DECLARE v_nome_leitor VARCHAR(250);
    DECLARE v_titulo_obra VARCHAR(500);
    SELECT nome_usuario INTO v_nome_leitor FROM usuario WHERE id = NEW.usuario_id;
    SELECT o.titulo INTO v_titulo_obra FROM exemplar e JOIN obra o ON e.obra_id = o.id WHERE e.id = NEW.exemplar_id;
    INSERT INTO log_auditoria (acao, usuario_id, usuario_nome, exemplar_id, titulo_exemplar, detalhes, data_hora)
    VALUES ('ATUALIZAÇÃO', NEW.usuario_id, CURRENT_USER(), NEW.exemplar_id, v_titulo_obra, CONCAT('Leitor: ', v_nome_leitor), NOW());
END $$

CREATE TRIGGER trg_auditoria_emprestimo_delete AFTER DELETE ON emprestimo FOR EACH ROW
BEGIN
    DECLARE v_nome_leitor VARCHAR(250);
    DECLARE v_titulo_obra VARCHAR(500);
    SELECT nome_usuario INTO v_nome_leitor FROM usuario WHERE id = OLD.usuario_id;
    SELECT o.titulo INTO v_titulo_obra FROM exemplar e JOIN obra o ON e.obra_id = o.id WHERE e.id = OLD.exemplar_id;
    INSERT INTO log_auditoria (acao, usuario_id, usuario_nome, exemplar_id, titulo_exemplar, detalhes, data_hora)
    VALUES ('EXCLUSÃO', OLD.usuario_id, CURRENT_USER(), OLD.exemplar_id, v_titulo_obra, CONCAT('Excluído registro do leitor: ', v_nome_leitor), NOW());
END $$
